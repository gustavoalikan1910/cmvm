...
### 4. Operações em Produção (Oracle Cloud)
- **Conexão SSH:** Utilize o `plink.exe` com a chave privada em `C:\Users\GustavoAlikanFeverei\Downloads\private_key_oracle.ppk`.
- **Host:** `ubuntu@163.176.241.242`.
- **Diretório do Projeto:** `~/CVMC-data`.
- **Manutenção:** Capacidade de verificar containers Docker, logs de serviços e realizar operações Git diretamente no servidor para deploy e hotfixes.
...
- Realizar operações de banco de dados ou Git (Local e Remoto/Produção).
- Gerenciar o ambiente de produção na Oracle Cloud via SSH.
